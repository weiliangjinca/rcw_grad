...
def emissivity_f(dofold,ctrl):
    ...
    for i in range(Nlayer):
        ...
        vals = vals + np.real(obj.Volume_integralpart2(obj.a0,obj.bN,FMFL[i],MtL[i], solveinsideL[i], normalize=1))*np.imag(epsdiff[i])
    ...
    val = vals*np.real(obj.omega)*np.sin(theta)*np.cos(theta)/np.pi
